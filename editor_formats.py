...
Also return, per side, "portrait_subjects_left" / "portrait_subjects_right": every specific named real person whose face that side's photograph would show, names copied VERBATIM from the headline, the supplied description, or the user's explicit input (no title, no organisation), at most three per side; an empty array when no personal name appears in that material. Never infer a name from a title, event, country, organisation or common knowledge. ONE EXCEPTION: when the material carries a block headed "Abbreviation glossary supplied by the newsroom", the personal names listed there are established fact handed to you by the newsroom, not something you inferred — treat each as if written out in full at the place the abbreviation appears, design their face in, and list them here. Do not name such a person in the text while leaving them out of this array. The exception reaches only the names printed in that block. "portrait_subjects_left_en" / "portrait_subjects_right_en": the same people, same order, same length, each copied VERBATIM from that material when an English or original-Latin spelling is present; empty string when it is not. Never fill an English name from Wikipedia, translation or common knowledge. "portrait_subjects_left_en_guess" / "portrait_subjects_right_en_guess": same order and length; only when the source does not spell the English name but its explicit name, title, nationality and role identify the person, give your best English-name guess, otherwise an empty string. These guess fields are photo-lookup metadata only: NEVER use them in a description or any rendered text.
"""
...
COVER_VISUAL_SCHEMA = {
    ...
    "properties": {
        ...
        "portrait_subjects_left_en_guess": {"type": "array", "items": {"type": "string"}},
        "portrait_subjects_right": {"type": "array", "items": {"type": "string"}},
        "portrait_subjects_right_en": {"type": "array", "items": {"type": "string"}},
        "portrait_subjects_right_en_guess": {"type": "array", "items": {"type": "string"}},
    },
    "required": [
        "visual_left", "visual_right",
        "portrait_subjects_left", "portrait_subjects_left_en", "portrait_subjects_left_en_guess",
        "portrait_subjects_right", "portrait_subjects_right_en", "portrait_subjects_right_en_guess",
    ],
    "additionalProperties": False,
}
...
3. "portrait_subjects" — every specific named real person whose face the photograph would show, names copied VERBATIM from the headline or the user's explicit input (no title, no organisation), at most three. Empty array when no personal name appears in that material. Never infer a name from a title, event, country, organisation or common knowledge. ONE EXCEPTION: when the material carries a block headed "Abbreviation glossary supplied by the newsroom", the personal names listed there are established fact handed to you by the newsroom, not something you inferred — treat each as if written out in full at the place the abbreviation appears, design their face in, and list them here. Do not name such a person in the text while leaving them out of this array. The exception reaches only the names printed in that block. "portrait_subjects_en" — the same people, same order, same length, each copied VERBATIM from that material when an English or original-Latin spelling is present; empty string when it is not. Never fill an English name from Wikipedia, translation or common knowledge. "portrait_subjects_en_guess" — same order and length; only when the source does not spell the English name but its explicit name, title, nationality and role identify the person, give your best English-name guess, otherwise an empty string. This guess is photo-lookup metadata only: NEVER use it in the visual description or rendered text.
"""
...
YT_COVER_DERIVE_SCHEMA = {
    ...
    "required": [
        "line1", "line2", "visual", "portrait_subjects", "portrait_subjects_en",
