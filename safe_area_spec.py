...
REPORTER_PROFILE = "記者"
EDITOR_PROFILE = "編輯"
# 編輯的第二種框（2026-08-19 使用者指定）：四邊各留畫布的 3%，內容等比例置入。
# 同日調過兩次：2%（38/22）→ 4%（77/43）→ 3%。改百分比只要動 PROFILES 這一筆
...
#
# 編輯安全框：1920×1080 的 3% 是 57.6／32.4，取整成 58／32，內容區 1804×1016。
# 這個內容區的長寬比 1.7756 與 16:9（1.7778）幾乎相同，所以 16:9 生成圖 FIT
# 進去零裁切、零拉伸，四周只剩那圈 3% 由襯底填滿——不像對位框（1.892）必須
# 在裁切／拉伸／補邊三者間二選一。
#
