...
REPORTER_PROFILE = "記者"
EDITOR_PROFILE = "編輯"
# 編輯的第二種框（2026-08-19 使用者指定）：四邊各留畫布的 4%，內容等比例置入。
# 調整歷程：2026-08-19 當日 2%（38/22）→ 4%（77/43）→ 3%（58/32），
# 2026-08-20 使用者再裁決改回 4%。改百分比只要動 PROFILES 這一筆
# 與測試的 THIN_FRAME_RATIO，投影停用規則是通則、會自動跟著適用。
...
#
# 編輯安全框：1920×1080 的 4% 是 76.8／43.2，取整成 77／43，內容區 1766×994。
# 這個內容區的長寬比 1.7767 與 16:9（1.7778）幾乎相同，所以 16:9 生成圖 FIT
# 進去零裁切、零拉伸，四周只剩那圈 4% 由襯底填滿——不像對位框（1.892）必須
# 在裁切／拉伸／補邊三者間二選一。
#
# 試過的三個值都零裁切，與 16:9 的比例差分別是 2%→0.0021、3%→0.0022、4%→0.0011。
# 差距都遠小於容差，選幾 % 是視覺取捨而非幾何限制。
PROFILES = {
