...
| 項目 | 值 | 來源 |
|---|---|---|
| 帶高 | `COVER_AI_HEADER_RATIO = 0.12` | 使用者「藍框稍微變大一點點」；同時是 prompt 的數字、量不到的退路、補帶的目標 |
| Logo 高 | 內高 × 0.70（720 → 53） | 內高 = 帶高 −12%（`COVER_AI_HEADER_CLEARANCE`） |
| 節目標籤高 | 內高 × 0.80（720 → 61） | 同上；不得小於 09-03 的 58 |
