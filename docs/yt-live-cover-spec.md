...
**2026-09-14 更新（兩次）**：早上裁決改由「創意」拉桿決定——創意 0 標題**預設程式壓字**
（零錯字、原圖零漂移），1 級起才交 AI；上面那句「預設 AI 生成」自此只適用於創意 1 以上。
同日晚放寬：創意 0 的勾選框**可以自己勾**（預設不勾），勾了就照辦；1 級以上鎖成必勾。
請求欄位 `title_mode` 省略（None）＝依創意等級取預設，明送 `ai`／`composite` 一律照辦
（見 `editor_formats.title_mode_for_creativity`）。
...
## 2026-09-09（第四批）：底色框給回數字、兩行標共用字級、蓋章 OFF 改用 `<底帶>`

第三批的底色框與蓋章 OFF 底帶**都沒有做成**，使用者實測附圖為證。兩處的教訓不同：

### 底色框：拿掉數字反而更糟

第三批的判斷是「模型跟得動關係式、跟不動百分比」，於是把百分比整個拿掉。結果模型
手上只剩模板那句 `lower 40% of the frame`，照抄，框從 58% 起跳、兩行字都蓋進去。

第四批把合成版的真實數字寫回去，**數字與關係式並存**：上緣 78%
（＝`compose.YT_BAND_TOP_RATIO` 0.778）、框只佔下方 22%；關係式改用**顏色**點名
（上緣齊白字基線、全飽和處在金黃字上緣），並明列否定（不是 lower third、不是
lower 40%、不是半個畫面）。條文裡的數字由測試直接對 `YT_BAND_TOP_RATIO` 驗，
不會兩邊各寫各的。

| 版本 | 條文怎麼寫 | 實測框上緣 |
| --- | --- | --- |
| 第一批 | 只有關係式，排在標題條**前面** | 0.58 |
| 第三批 | 關係式 ＋ OVERRIDE，移到標題條之後 | 0.70 |
| 第四批 | 上面那些 ＋ 明寫 78%／下方 22% ＋ 顏色點名 | 0.76（實測，使用者驗收通過） |
| 第五批 | 同上，但百分比改成從 `compose.YT_BAND_TOP_RATIO` 算 | 未再驗（只有數字從 78→77） |
| 合成版 | `YT_BAND_TOP_RATIO` | 0.778 → **0.770**（第五批） |

### 兩行標：字級要共用，不是各自撐滿

三個 AI 模板都寫著每一行 `filling／spanning almost the full width`——那是叫每一行
各自撐滿，字少的那行就被放大，兩行差一截（使用者附圖）。合成版沒這個問題，
`compose._yt_shared_title_font` 本來就兩行共用一個字級。

改成：共用一個字級，由**較長那行**決定；較短那行照同一個字級排，自然變窄留白，
不准放大去對齊寬度。國內外／熱搜／整點三個模板一起改。
（同一輪使用者撤回了「再稍微縮小一點點」，`YT_TITLE_SIZE_RATIO` 不動。）

### 播出鏡面蓋章 OFF：要有標記，不能只講「最後一張卡」

第三批叫模型「把最後一張 `[內文小標]` 下移到底帶」。那張卡在 `variable` 裡跟其他卡
一模一樣，模型挑不出來，四張一起疊在半邊，底部全空。蓋章 ON 做得到，是因為
`<蓋章>` 是 `variable` 裡看得見的**標記**。

第四批比照辦理，給底帶自己的標記 `<底帶>`（`variable` 的最後一行），並在
`main.ensure_bottom_band_line` 做確定性兜底——模型漏寫就把最後一張卡升級。
同時修第 1 條：挖空框是寬扁的 16:9 橫幅視窗、垂直置中、**不到畫面底部**，
舊句「filling most of the half」讓模型畫成整片高牆，底下那條帶根本不存在。
使用者同時開放底帶跨版（「就像標題可跨版」）。

### 消化速度

規則只會愈加愈多（這一批第 4 項就在加），所以不走刪規則那條路。改成把**思考**封頂
（OpenRouter `reasoning.max_tokens`，預設 2000）＋給重試迴圈一個 230 秒的牆鐘期限，
在 Cloud Run 的 300 秒硬砍之前回一句看得懂的話。理由與量測見
`docs/plan-20260909b-user-feedback.md` 第 5 項。

> ⚠ **2026-09-16 更正（D21）**：上面那句「思考封頂＝`reasoning.max_tokens`，預設 2000」
> **已經不是現況，而且當初根本沒生效**——實測 Sonnet 5 會忽略這個欄位（送 2000、
> 實際吃 4005–11999）。現行做法是送 `reasoning.effort`（預設 `low`），
> 主消化模型也已從 `anthropic/claude-sonnet-5` 換成 `google/gemini-3.8-flash`。
> 230 秒牆鐘期限這部分不變。本文其餘段落提到 Claude 為正式站模型的地方，
> 均為 2026-09-16 之前的歷史記載。


## 2026-09-09（第五批）：行距再縮，羽化只能往上長

使用者要兩件互相擠的事：**行距再縮**，而且**底色框邊緣再多一點漸層羽化**。
斜坡本來就住在兩行字中間那道空隙裡，空隙一縮，羽化更沒地方長。往下延會切到黃字
（0908 的裁決：不超過第二行標題），所以改成**往上多起跑一段**：

```
YT_LINE1_BASELINE_RATIO 0.778 → 0.790      # 行距 0.180 → 0.168，第二行貼底不動
YT_BAND_LEAD_RATIO      0.020              # 斜坡比第一行基線再高這麼多起跑
YT_BAND_TOP_RATIO       = 0.790 - 0.020 = 0.770
YT_BAND_FADE_RATIO      0.0365 → 0.052     # 斜坡 26px → 37px；0.822 到全濃度
```

smoothstep 起頭幾乎是 0，白字墨水上緣算出來不到 8% 濃度——那一段藏在白字腳下看不出來。
全濃度處 0.822 仍壓在黃字墨水上緣 0.8231 之上。

**只動國內外新聞直播與今日熱搜。** 整點直播的兩行行距本來就是 0.150（比新值更緊），
十點不一樣是白／黃／紅三行、走 `COVER_TITLE_LINE_GAP`——使用者講的是兩行標。

條文裡的百分比同時改成從 `compose.YT_BAND_TOP_RATIO` 算。第三批的教訓是
「模型手上有什麼數字就抄什麼」，所以那個數字絕不能是手抄的、會過期的。

## 十點不一樣・AI 版標頭帶（2026-09-09 第二輪）

| 項目 | 值 | 來源 |
|---|---|---|
| 帶高 | `COVER_AI_HEADER_RATIO = COVER_HEADER_RATIO`（0.105，720 → 76） | 使用者「藍框稍微變大一點點」；直接借合成版的，同時是 prompt 的數字、量不到的退路、補帶的目標 |
| Logo 高 | 帶高 × 0.70（720 → 53） | 與合成版同一套；不得小於 09-03 的 50 |
| 節目標籤高 | 帶高 × 0.80（720 → 61） | 同上；不得小於 09-03 的 58 |
| 補帶 | `ensure_ai_header_band` | 模型畫太薄才補；量不到不補 |

模型實際畫出來的帶約 8.1–8.6%，比 prompt 要的薄，所以補帶這一步是常態不是例外。
