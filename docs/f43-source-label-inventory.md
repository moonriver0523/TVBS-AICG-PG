...
## B107 更新（2026-09-26 使用者裁決）

B107 推翻本文件中「`mode="ai"`／`title_mode="ai"` 因 AI 畫標題就一律標
`AI示意圖`」的部分結論。封面標籤改為只看**底圖素材來源**：所有實際使用的圖片皆為
使用者上傳的 `asis`、沒有缺圖由 AI 生背景、也沒有 `aiedit` 時，不論 AI 標題開關或
創意度，一律不標 `AI示意圖`。十點雙切按左右格分別判定；YT 四版型與十點滿版只要
任一底圖素材由 AI 生成／修改，仍標整張（十點雙切則只標該側）。refine 是 AI 修改，
重組回封面後仍須標示。這次裁決只改 AI 標籤 gate，不新增封面來源欄位，也不擴張既有
「畫面來源」標籤的適用路徑；下文保留作為 F43 當時的歷史盤點。
