...
## 標題創意拉桿（2026-09-08 開關 → 2026-09-09 五段，只有 AI 整張版）

`title_creativity` 0–4，預設 0；`editor_formats.cover_ai_title_style_clause(level)`
決定 TYPOGRAPHY 段尾追加哪一段。0 完全不追加＝現行排版。
1 放開字體／描邊／材質；2 是整組 house style（1.5–2 倍落差、關鍵詞壓框、
飽和平塗），**版位仍固定**；3 再加版位自由與 1–2 個無字小圖示；
4 再加幅度（2.5–3 倍落差、多層描邊立體、傾斜約 8 度、錯落與輕微疊字、
爆裂與速度線、圖示上限 3）。

級距是 2026-09-09 實拍 0–4 梯子後由使用者裁定的（舊 2、3 差距不大，融成一級）。

FIXED (a)–(g)（一字不改、正體中文、程式後貼的三塊區域、不得跨格）**每一級都原樣附上**，
不隨等級鬆綁。舊的 `title_style: plain | designed` 保留為別名，對應 0 與 4。

### （舊）設計標題開關（2026-09-08，只有 AI 整張版）
