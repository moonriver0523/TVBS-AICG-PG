...
- **標題（雙切，2026-09-08 大改）**：各格下緣，左格靠左、右格靠右，最多 **4 行**
  ...
  - 配套是把行拆得夠短，共同字級才撐得起來：寬度規則（`_wrap_pairs`，起始字級塞不進格寬就
    從中間拆）之外再加字數規則（`_fill_pairs`，一行超過 `COVER_TITLE_FILL_MIN_CHARS`＝7 字
    就再拆），最長的先拆，上限 4 行。
  - 垂直：4 行 × 字級 × 行距 1.06 必須落在標頭帶以下（`_cover_title_vertical_cap`），
    塞不下就兩格一起縮。拆完縮到最小字級仍超出寬度就 `ComposeError`→400。
  - **配色依「段落」不依「行序」**（使用者 2026-09-07 明講的規則：第一個空格後是黃字、
    第二個空格後是紅字）。行帶著段落索引一起走（`cover_title_line_pairs` 回
    `list[tuple[str, int]]`），拆出來的行繼承原段落的顏色，所以**連續兩行同色是正常的**。
    第 1 段白、第 2 段黃、第 3 段紅（紅字白描邊，其餘深色描邊），外加陰影。
    兩段的標題就沒有紅字——紅字靠「AI 消化標題」一律出 3 段（已上線）。
  - 例：左「澳洲擬立新法 民眾可關閉社群媒體演算法」→ 白／黃／黃 3 行；
    右「菲律賓前眾議長涉貪被捕 與總統小馬可仕為表兄弟」→ 白／白／黃／黃 4 行；兩格同字級。
...
- 那一段只寫「怎麼排」，並自己重申一次「照給定的行逐字印、不得增減／重排／重拆」。
  **設計感只用字級表現**（使用者 2026-09-08 裁決）：關鍵數字／關鍵詞放大、緊密堆疊、粗描邊＋陰影；
  明文禁止模型自己加黃紅強調色——顏色一律照逐行標記（依段落）走。
