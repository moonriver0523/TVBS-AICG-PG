...
- **標題**：各格下緣，左格靠左、右格靠右，最多 3 行由下往上堆，**逐行各自撐滿格寬**（短行大、長行小，
  ...
  **補到 3 行（2026-09-08，雙切）**：`wrap_cover_title_lines` 只在塞不進格寬時才拆，兩行都塞得進就
  維持兩行，第三行的紅字永遠不出現。所以在寬度規則之外再補一條純字數規則 `_fill_cover_title_lines`：
  行數不足 3 行時，把最長的一行（≥ `COVER_TITLE_FILL_MIN_CHARS`＝9 字）再從中間拆一次，重複到湊滿
  或沒有夠長的行。合成版與 AI 版（`cover_title_lines`）共用同一支，斷句才不會又分歧；滿版不套。
  同日 `COVER_TITLE_WIDTH_RATIO` 0.84→0.90（版位放寬，塞不進起始字級的行縮完更大）。
  副作用：`tests/test_ten_cover_title_fit.py` 的「真的塞不下」樣本要跟著加長，32 字在 0.90 下已塞得進。
