...
### 8.8 使用者裁決（2026-09-21，第二批）

**裁決 8（對應 8.7 第 1 項）：`yt_vstrip` 標為 N/A，不動。**
採納建議解讀——裁決 4 的「全部版型都開」讀作「所有**會生背景圖**的版型都開」。
直標維持純 compose 透明 PNG，不新增背景生圖，原文仍只用於既有標題與來源消化。
驗收矩陣中 `yt_vstrip` 一列標 `N/A（無生圖模型）`，不列入 A/B 樣本。

因此 `D22-PR1` 的實際涵蓋範圍確定為：一般 CG、播出鏡面、十點封面（滿版與 split）、
四種 YT 封面，共六條路徑；AI 標題層依裁決 7 硬排除，`yt_vstrip` 依本項標 N/A。
