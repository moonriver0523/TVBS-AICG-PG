...
11. LIST THE PLACES YOU WANT MARKED, SO THE PROGRAM CAN LOOK THEM UP. Put every place that should carry a marker into the "map_places" field, one entry each, in the order you want them read. Write each entry as a lookup query, not as a caption: include the city and district that disambiguate it (「基隆市 西定路」, not 「西定路」), because a bare street or hill name matches dozens of places nationwide. Use the place's real name, never a description. If the graphic is not a map, leave the array empty. The program geocodes these names against a real gazetteer and may attach a real basemap with the markers already drawn — that is why the entries must be findable names rather than pretty labels. Nothing you write in this field is ever printed on the graphic — it is a lookup query and only that. The name the viewer reads is governed by rule 16. ONLY LOOKUPABLE POINTS BELONG IN THIS FIELD. A gazetteer holds named points and named administrative areas — nothing else. Never put in a loose region or direction (「北海岸」「南部」「東海岸」「北台灣」「市區」「低窪地區」), and never a position expressed as a distance along a road (「楊梅路段北向68公里」「國道1號中壢路段」): the gazetteer has no such entry, so it either returns nothing or — worse — matches an unrelated shop that happens to share the words, and that wrong point gets drawn on the basemap. If the place you mean cannot be written as a named point or a named administrative area, name the district it falls in instead (「桃園市 楊梅區」), or leave it out of this field entirely and describe it in "structure" as a schematic position. Leaving it out costs you a marker; a wrong lookup puts a marker on the wrong town. A NAMED FACILITY IS LOOKED UP BY ITS OWN NAME, NOT BY THE ROAD IT SITS ON. Interchanges, stations, ports and airports are named points the gazetteer holds under their own names: write 「中壢交流道」, never 「國道1號中壢交流道」 — the road prefix is not part of the name and makes the entry unfindable. If the bare name could be ambiguous, put the city and district in front of it instead (「桃園市 中壢區 中壢交流道」), which stays findable.
...
14. THE MARKERS ARE THE PLACE LABELS — NEVER BUILD A LEGEND. Every place you mark already carries its own name beside its marker on the map. Do not write those same place names a second time as a 內文小標 line, a caption list, a key, a legend, a 圖例 panel or a marker index: on screen that renders as a separate box repeating what the map already says, and it eats the space the map needs. Never ask for a legend box, a key panel or a colour-code panel of any kind — the only map furniture allowed is the north arrow and scale bar of rule 2. The 內文小標 lines are for the news itself (what happened at those places, when, how serious), never for a list of the places.
15. NEVER LET AN INSTRUCTION WORD BECOME PRINTED TEXT. 標示, 標出, 請標, 標記, 位置如下 and the like are directions about what to do with the map, not wording for the graphic to display. The renderer prints "variable" verbatim, so an instruction word left in there comes out on the finished graphic as a caption reading 「標示 基隆廟口」. Write the place name on its own, with no verb in front of it.
16. ONE PLACE, ONE NAME ON SCREEN. "map_places" is a lookup query and may need the gazetteer's official full form (「基隆市 基隆廟口夜市」) to be findable. Everything the viewer reads must instead use the short name the story itself uses (「基隆廟口」), and "structure" and "variable" must use that same short form as each other. Never carry the gazetteer form into "structure" or "variable": that is exactly how one graphic ends up labelling the same place 基隆廟口夜市 on the map and 基隆廟口 in the text.
"""
