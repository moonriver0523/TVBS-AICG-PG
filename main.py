...
# 編輯與記者在這裡分道：記者維持整段滿版（21:9 FIT 置框，塞滿才不浪費畫面）。
# 編輯改成「背景滿版、元素留邊」——2026-08-17 稽核發現原本這段強制 STRUCTURE 以
# "uses the entire frame edge to edge" 開頭，與生圖階段的 EDITOR_FULL_BLEED_RULES
# （要求上下留背景帶）自相矛盾，模型每次選擇聽哪一邊，實測結果從 0px 跳到 25px。
EDITOR_LAYOUT_FULL_BLEED = """   - EDGE-SAFE LAYOUT (NON-NEGOTIABLE): the structure description MUST begin with this exact sentence: "The background is one single continuous image running to all four edges, while the infographic itself sits within it: a clear strip of plain background about as tall as one headline character runs across the top and across the bottom, and no element enters either strip." After that sentence, every element you place MUST be positioned using ONLY qualitative spatial words (e.g. "across the upper area", "centred", "along the right side", "with clear separation from its neighbours"). NEVER express any position, inset, gutter, margin, or size as a percentage, pixel, ratio, or number of any kind — those figures get drawn as visible text labels in the final image. Never draw a visible border or letterboxed area, and never scale the design down into a small central box — the strips are plain background, not a frame. The <蓋章> stamp banner and any data-source line are the LOWEST ROW OF THE DESIGN, sitting clearly above the bottom strip with plain background visible beneath."""
