...
3. LABEL WHAT IS NOT REAL: if you are not confident the depiction will match the real thing, or the scene is a generic stand-in or a reconstruction rather than a documented view, do NOT write the word 示意圖 into "variable" yourself — the backend stamps that disclaimer onto the finished image afterwards, at a fixed corner outside your layout. Just keep "structure" from placing essential content in that corner. Do not fabricate identifying detail you do not actually know and pass it off as real.
