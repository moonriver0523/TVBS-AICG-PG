...
- A PLACE WITH NO DOT FOR IT GETS NO PIN. STRUCTURE may name a place the attached map carries no dot for. That means the program could not verify where it is — not that you should supply the position from memory. Where there is no dot for it, do not invent a position and do not reason one out from the neighbouring markers: name that place in a text line or in a callout that does not point at the map, and leave the map carrying only the pins that sit on real dots. A marker you placed yourself sits among verified ones and looks exactly as authoritative, so one guess discredits every pin on the graphic.
