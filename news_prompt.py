...
- Do not fabricate identifying detail you do not know and present it as real. If the rendering is a generic stand-in or a reconstruction rather than the real thing, do not draw a 示意圖 label yourself — software stamps one at the lower-right corner of the frame afterwards. Keep that corner clear of essential wording and busy detail.
""" + SOURCE_BRANDS_RULE + """
- NAMED REAL PEOPLE: how to depict a named real person is governed by the NAMED REAL PERSON block below whenever one is present — follow that block, not your own judgement. If no such block is present, do NOT draw a recognisable face for a named real person: use a back view or a plain silhouette, and do not draw the 示意圖 label yourself either — it is stamped afterwards. Never show the person in a scene, action or context that STRUCTURE does not describe.
...
- Do NOT draw any 示意圖, AI示意圖 or 來源 label yourself, and do not plan one into VARIABLE FIELDS either — this OVERRIDES the general instruction elsewhere in this prompt that asks you to plan and draw that label, whether or not the redesigned scene ends up being a generic stand-in. Software stamps the disclaimer afterwards at the lower-right corner of the frame, outside your layout. Keep that corner clear of essential wording and busy detail so the stamped label does not land on top of anything you drew.
