...
class EditorAdaptiveShaveTests(unittest.TestCase):
    """裁掉模型自己留的純背景，換取更小的水平拉伸失真。

    最重要的保證是「絕不切到內容」：偵測從邊界往內走，一遇元素就停。
    偵測失敗的代價只是失真回到全額，不會產生破圖。
    """

    ZONE = (1748, 924)

    def _image(self, size, margin, bg=(9, 14, 32), fg=(240, 200, 40)):
        """上下各 margin 列純背景、中間是元素的測試圖。

        元素刻意畫成間隔色塊而非整片實心——真實 CG 的內容區不會是「整列同色」，
        而偵測器正是靠「這列不均勻」判斷有元素。畫成實心會讓假圖比真圖更難測。
        """
        img = Image.new("RGB", size, bg)
        draw = ImageDraw.Draw(img)
        if margin < size[1] / 2:
            for x in range(20, size[0] - 20, 120):
                draw.rectangle([x, margin, x + 70, size[1] - 1 - margin], fill=fg)
        return img

    def test_detects_the_margin_the_model_left(self):
        for margin in (0, 5, 18, 40):
            with self.subTest(margin=margin):
                img = self._image((1536, 864), margin)
                limit = 100
                self.assertEqual(
                    safe_frame.plain_background_margin(img, limit), min(margin, limit)
                )

    def test_never_shaves_past_zero_distortion(self):
        """裁到來源比例＝安全區比例就該停，再裁只是白白丟畫面。"""
        img = self._image((1536, 864), 200)      # 留白遠多於需要
        shaved = safe_frame._shave_plain_edges(img, self.ZONE)
        ratio = shaved.width / shaved.height
        self.assertAlmostEqual(ratio, self.ZONE[0] / self.ZONE[1], places=2)
        self.assertGreaterEqual(shaved.height, 800)

    def test_no_margin_means_no_shave(self):
        img = self._image((1536, 864), 0)
        self.assertEqual(safe_frame._shave_plain_edges(img, self.ZONE).size, (1536, 864))

    def test_shaving_reduces_distortion_without_touching_content(self):
        margin = 23      # 2026-08-17 實測模型在有呼吸邊指示時留的量
        img = self._image((1536, 864), margin)
        shaved = safe_frame._shave_plain_edges(img, self.ZONE)
        self.assertEqual(shaved.size, (1536, 864 - 2 * margin))
        before = (self.ZONE[0] / self.ZONE[1]) / (1536 / 864) - 1
        after = (self.ZONE[0] / self.ZONE[1]) / (shaved.width / shaved.height) - 1
        self.assertAlmostEqual(before, 0.064, places=3)
        self.assertLess(abs(after), 0.015, "裁掉純背景後失真應降到 1.5% 以內")
        # 裁完後第一列就該是內容那一列（元素起點），代表一列背景都沒多留、也沒少切
        self.assertEqual(shaved.getpixel((40, 0)), (240, 200, 40))

    def test_full_width_solid_band_at_the_edge_is_not_mistaken_for_background(self):
        """貼邊的實心色條整列同色，光看『這列夠均勻』會誤判成背景。"""
        img = self._image((1536, 864), 20)
        ImageDraw.Draw(img).rectangle([0, 0, 1535, 9], fill=(200, 30, 30))
        self.assertEqual(safe_frame.plain_background_margin(img, 100), 0)

    def test_element_touching_the_edge_falls_back_to_full_stretch(self):
        """模型把元素畫到貼邊時，寧可全額拉伸也不能裁。"""
        img = self._image((1536, 864), 20)
        ImageDraw.Draw(img).rectangle([700, 0, 760, 3], fill=(255, 0, 0))  # 貼上緣的小圖示
        self.assertEqual(safe_frame.plain_background_margin(img, 100), 0)
        self.assertEqual(safe_frame._shave_plain_edges(img, self.ZONE).size, (1536, 864))
    ...
    def test_asymmetric_margins_use_the_smaller_side(self):
        ...
        draw = ImageDraw.Draw(img)
        for x in range(20, 1516, 120):          # 上緣留 30、下緣只留 10
            draw.rectangle([x, 30, x + 70, 853], fill=(240, 200, 40))
        self.assertEqual(safe_frame.plain_background_margin(img, 100), 10)
